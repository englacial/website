...
date: "2026-04-27"
image: images/antarctic_lines.png
jupyter: python3
format:
    html:
...
When you create an `OPRConnection`, xOPR syncs a local copy of the STAC catalog — the index of all available data. After the first sync, subsequent startups check an ETag and only download changes, so it's fast.

Here are all of the Antarctic radar lines available in the `xOPR` catalog (so far). (Don't worry — we do Greenland too!) If you haven't checked in on OPR in a while, you might be surprised by how much ICECAP data from the University of Texas Institute for Geophysics is now included.
...
If you want to pull data for your favorite glacier, we make that easy too. You can query the catalog by many criteria, including by passing a geometry. `xOPR` also ships with a helper to load named regions from the [MEaSUREs Antarctic Boundaries](https://nsidc.org/data/nsidc-0709) dataset, so you can just ask for a region by name.
...
```{python}
#| label: fig-radargram
#| fig-cap: "Radar echogram over Totten Glacier. Surface and bed layer picks (dashed lines) are loaded directly from the OPR layer database."

# Resample to uniform slow_time spacing (required for imshow)
stacked = frame.resample(slow_time='2s').mean()
data_dB = 10 * np.log10(np.abs(stacked['Data']))

# Load layer picks (surface + bed)
layers = opr.get_layers(stacked)

fig, ax = plt.subplots(figsize=(8, 3.5))
data_dB.plot.imshow(x='slow_time', cmap='gray', ax=ax,
                            vmin=np.nanpercentile(data_dB.values, 5),
                            vmax=np.nanpercentile(data_dB.values, 99),
                            )
ax.invert_yaxis()

layer_style = {'linewidth': 1.2, 'linestyle': '--'}
layer_labels = {'standard:surface': 'Surface', 'standard:bottom': 'Bed'}
layer_colors = {'standard:surface': '#4fc3f7', 'standard:bottom': '#ef9a9a'}
...
if layers:
    for layer_name, layer_ds in layers.items():
        ...
        layer_ds['twtt'].plot(ax=ax, x='slow_time', label=label,
                              color=color, **layer_style)
